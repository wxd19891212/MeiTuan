...
@interface JGDiscountCell () {
    NSMutableArray *_array;
}

@end

@implementation JGDiscountCell

+(instancetype)cellWithTableView:(UITableView *)tableView {
   
    static NSString *cellIndentifier = @"discountcell";
    JGDiscountCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
    if (cell == nil) {
        cell = [[JGDiscountCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellIndentifier];
    }
...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        for (int i = 0; i < 4; i++) {
            //背景
            UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(i*kDeviceWidth/2, 0, kDeviceWidth/2, 80)];
            if (i<2) {
                backView.frame = CGRectMake(i*kDeviceWidth/2, 0, kDeviceWidth/2, 80);
            }else{
                backView.frame = CGRectMake((i-2)*kDeviceWidth/2, 80, kDeviceWidth/2, 80);
            }
            backView.tag = 100+i;
            backView.layer.borderWidth = 0.25;
            backView.layer.borderColor = [JGSeparaterColor CGColor];
            ...
            //标题
            UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 20, kDeviceWidth/2-10-60, 30)];
            ...
            //子标题
            UILabel *subTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 40, kDeviceWidth/2-10-60, 30)];
            ...
            //图
            UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(kDeviceWidth/2-10-60, 10, 60, 60)];
            imageView.tag = 240+i;
...
-(void)setDiscountArray:(NSMutableArray *)discountArray{
    ...
    for (int i = 0; i < discountArray.count; i++) {
        JGDiscountModel *discount = discountArray[i];
    ...
    for (int j = 0; j < 4; ++j) {
        ...
        JGDiscountModel *discount = _array[j];
...
-(void)OnTapBackView:(UITapGestureRecognizer *)sender{
    ...
    JGDiscountModel *discount = _array[index];
...
//        NSLog(@"%@",str);
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didSelectUrl:withType:withId:withTitle:)]) {
